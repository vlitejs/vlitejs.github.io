...
<Aside type="note">- _`vlitejs 5.0.1`_ - `plyr 3.7.8` - _`videojs 8.3.0`_</Aside>
...
<Aside type="caution">
	> - _&sup1; Video only._ > - _&sup2; Vimeo provider requires a Plus or Pro account to use this
	feature (see [Vimeo
	plans](https://vimeo.zendesk.com/hc/en-us/articles/228068968-Compare-Vimeo-plans))._ > - _&sup3;
	See [Youtube embed options](https://developers.google.com/youtube/player_parameters#Parameters)
	and [Vimeo embed options](https://github.com/vimeo/player.js/#embed-options)._ > - _⁴ Fullscreen
	API not yet supported on iPhone._ > > The `autoplay` parameter automatically activates the `muted`
	option because the API can only be initiated by a user gesture (see [Autoplay policy
...
<Aside type="tip">The `onReady` function can also be written with an arrow function.</Aside>
...
<Aside type="note">_&sup1; Video only._</Aside>
